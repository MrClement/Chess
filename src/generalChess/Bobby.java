...
public class Bobby {
	...
	public void getBoard(Board b) {
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				char type = b.getBoardArray()[x][y].toString().charAt(1);
				boolean col = b.getBoardArray()[x][y].getColor();
				switch (type) {
					case 'P':
	...
	// return moves in an array with the first row storing which piece it is and
	// then all the subsequent rows storing the possible moves for that piece
	...
	public ArrayList<ArrayList> getMoves() {
		...
		for (int t = 0; t < poss.size(); t++) {
			...
			if (type == 'K') {
				for (int x = (int) loc.getX() - 1; x <= (int) loc.getX() - 1; x++) {
					for (int y = (int) loc.getY() - 1; x <= (int) loc.getY() - 1; y++) {
						if ((int) loc.getX() - 1 > -1 && (int) loc.getX() + 1 < 8 && (int) loc.getY() - 1 > -1
								&& (int) loc.getY() + 1 < 8 && board[x][y].toString().charAt(1) == 'X'
			...
			if (type == 'N') {
				...
				// 2 up 1 right
				if (xCoord + 1 < 8 && yCoord < 8 && board[xCoord + 1][yCoord + 2].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord + 1, yCoord + 2);
					poss.get(t).add(p);
				}

				// 2 up 1 left
				if (xCoord - 1 > -1 && yCoord + 2 < 8 && board[xCoord - 1][yCoord + 2].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord - 1, yCoord + 2);
					poss.get(t).add(p);
				}
				...
				// 2 down 1 left
				if (xCoord - 1 > -1 && yCoord - 2 > -1 && board[xCoord - 1][yCoord - 2].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord - 1, yCoord - 2);
					poss.get(t).add(p);
				}
				...
				// 2 down 1 right
				if (xCoord + 1 < 8 && yCoord - 2 > -1 && board[xCoord + 1][yCoord - 2].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord + 1, yCoord - 2);
					poss.get(t).add(p);
				}
				...
				// 2 right 1 down
				if (xCoord + 2 < 8 && yCoord - 1 > -1 && board[xCoord + 2][yCoord - 1].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
				...
				// 2 right 1 up
				if (xCoord + 2 < 8 && yCoord + 1 < 8 && board[xCoord + 2][yCoord + 1].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord + 2, yCoord + 1);
					poss.get(t).add(p);
				}
				...
				// 2 left 1 down
				if (xCoord - 2 > -1 && yCoord - 1 > -1 && board[xCoord - 2][yCoord - 1].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord - 2, yCoord - 1);
					poss.get(t).add(p);
				}
				...
				// 2 left 1 up
				if (xCoord - 2 > -1 && yCoord + 1 < 8 && board[xCoord - 2][yCoord + 1].toString().charAt(1) == 'X'
						|| board[xCoord + 1][yCoord + 2].getColor() == !color) {
					Point p = new Point(xCoord - 2, yCoord + 1);
					poss.get(t).add(p);
				}
			...
			if (type == 'B') {
				...
				// as long the last point in each diagonal of the array is a
				// blank, the bishop will add the next point to its array

				if ((x + 1) < 8 && (y + 1) < 8)
					a.add(new Point(x + 1, y + 1));
				if ((x + 2) < 8
						&& (y + 2) < 8
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
				...
				if ((x + 3) < 8
						...
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
					a.add(new Point(x + 3, y + 3));
				if ((x + 4) < 8
						&& (y + 4) < 8
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
					a.add(new Point(x + 4, y + 4));
				if ((x + 5) < 8
						&& (y + 5) < 8
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
					a.add(new Point(x + 5, y + 5));
				if ((x + 6) < 8
						&& (y + 6) < 8
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
					a.add(new Point(x + 6, y + 6));
				if ((x + 7) < 8
						&& (y + 7) < 8
						&& board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].toString()
								.charAt(1) == 'X')
					a.add(new Point(x + 7, y + 7));

				// the 'adding' stops as soon as the bishop hits a board. this
				// removes the board from the possible moves array if it is of
				// the same color as our bishop
				if (board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].getColor() == color)
					a.remove(a.size() - 1);
			...
			if (type == 'R') {
				...
				// the 'adding' stops as soon as the rook hits a board. this
				// removes the board from the possible moves array if it is of
				// the same color as our rook
				if (board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].getColor() == color)
					a.remove(a.size() - 1);
			...
			if (type == 'Q') {
				...
				// the 'adding' stops as soon as the queen hits a board. this
				// removes the board from the possible moves array if it is of
				// the same color as our queen
				if (board[(int) (a.get(a.size() - 1).getX())][(int) (a.get(a.size() - 1).getY())].getColor() == color)
					a.remove(a.size() - 1);
				...
				// as long the last point in each diagonal of the array is a
				// blank, the queen will add the next point to its array
	...
	// makes the move you want - input a point with starting and ending
	// coordinates of move
	// note: assumes that the move you are making is valid and doesn't put you
	// into check
	public Piece[][] move(Point start, Point fin) {
		...
		// replaces the spot where you want to move with the piece you want to
		// move
	...
	public void isThreatened(Point p) {
		...
		// arraylist of the all the locations of the pieces that threaten your
		// piece
		ArrayList<Point> threats = new ArrayList<Point>();
		// go through each of the possible moves the enemy team can make by
		// going through the all
		...
		for (int i = 0; i < other.getMoves().size(); i++) {
			for (int k = 2; k < other.getMoves().get(i).size(); k++) {
				// create a new point with the location of the point where the
				// enemy piece can move
				Point temp = new Point((Point) other.getMoves().get(i).get(k));
				// if that location is the same as your piece, add that location
				// to the array
				if (temp.equals(p)) {
					threats.add(temp);
				}
	...
	public int checkMate() {
		...
		if ((index != -1) && (a.get(index).size() == 2))
			return checkmate = -1;
		// if size of array containing kings moves is 2 (first index being 'K'
		// and second index being current
		// position and that is it then you're in checkmate (assumes king wont
		// have a possibleMove() of going into check)
		...
		if ((index != -1) && (opponents.get(index).size() == 2))
			return checkmate = 1;
		// if size of array containing kings moves is 2 (first index being 'K'
		// and second index being current
		// position and that is it then you're in checkmate (assumes king wont
	...
	public static void main(String[] args) {
		...
		// if bishops exist check to make sure its okay if bishop can move to
		// 2,4

		if (index != -1) {
			for (int x = 0; x < a.get(index).size(); x++) {
				if (a.get(index).get(x).equals(new Point(2, 4)))
		...
		// if bishops exists check to make sure its okay if bishop can move to
		// 2,4
		...
		if (oppPieces[5][1].toString().charAt(0) == 'P')
			isblackpawnreadytogetpwned = true;
		index = -1;
		...
		if (index != -1) {
			for (int x = 0; x < a.get(index).size(); x++) {
				if (a.get(index).get(x).equals(new Point(5, 1)))
					BoardLords.move(new Point(5, 5), new Point(5, 1));
			}
		}

		// check for checkmate
		if (BoardLords.checkMate() == 1)
			System.out.println("WE WIN MOTHALICKAS");
		// cele hard or lose
	}
