		...
		for(int x = 0; x < 8; x++) {
			for(int y = 0; y < 8; y++) {
				...
				if(p instanceof Bishop) {
					DerpyBishop db = new DerpyBishop(pieceColor, new Point(x,y));
					arr[x][y] = db;
				}
				else if(p instanceof Blank) {
					DerpyBlank db = new DerpyBlank(new Point(x,y));
					arr[x][y] = db;
				}
				else if(p instanceof King) {
					DerpyKing db = new DerpyKing(pieceColor, new Point(x,y));
					arr[x][y] = db;
				}
				else if(p instanceof Knight) {
					DerpyKnight db = new DerpyKnight(pieceColor, new Point(x,y));
					arr[x][y] = db;
				}
				else if(p instanceof Pawn) {
					DerpyPawn db = new DerpyPawn(pieceColor, new Point(x,y));
					arr[x][y] = db;
				}
					...
					DerpyRook db = new DerpyRook(pieceColor, new Point(x,y));
					arr[x][y] = db;
				}
